 ...
 */
class RandomForestRegressor extends BaseTrain {
   ...
   * 模型描述
   */
  override def describe: String = "RandomForest(随机森林回归模型)"
  ...
  override def process(spark: SparkSession, df: Dataset[Row]): PipelineModel = {
    ...
    val randomForestRegressor = new regression.RandomForestRegressor()
      ...
      .setMaxMemoryInMB(config.getInt("maxMemoryInMB"))
      .setNumTrees(config.getInt("numTrees"))
    if (config.hasPath("featureSubsetStrategy")) {
      randomForestRegressor.setFeatureSubsetStrategy(config.getString("featureSubsetStrategy"))
    }

    if (config.hasPath("checkpointInterval")) {
      spark.sparkContext.setCheckpointDir(config.getString("checkpointDir"))
      randomForestRegressor.setCheckpointInterval(config.getInt("checkpointInterval"))
      randomForestRegressor.setCacheNodeIds(true)
    }

    if (config.hasPath("printParams") && config.getBoolean("printParams")) {
      println(">>>[INFO] 模型参数: ")
      println(randomForestRegressor.explainParams())
    }

    stages += randomForestRegressor

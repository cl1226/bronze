 ...
 */
class DecisionTreeRegression extends BaseTrain {
   ...
   * 模型描述
   */
  override def describe: String = "(DecisionTreeRegression)决策树回归模型"
  ...
  override def process(spark: SparkSession, df: Dataset[Row]): PipelineModel = {
    ...
    if (config.hasPath("checkpointInterval")) {
      ...
      decisionTreeRegressor.setCheckpointInterval(config.getInt("checkpointInterval"))
      decisionTreeRegressor.setCacheNodeIds(true)
    }

    if (config.hasPath("printParams") && config.getBoolean("printParams")) {
      println(">>>[INFO] 模型参数: ")
      println(decisionTreeRegressor.explainParams())
    }

    stages += decisionTreeRegressor

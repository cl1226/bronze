 ...
 */
class GBTClassifier extends BaseTrain {
   ...
   * 模型描述
   */
  override def describe: String = "GBTClassifier(梯度提升树分类模型)"
  ...
  override def process(spark: SparkSession, df: Dataset[Row]): PipelineModel = {
    ...
    gbtClassifier.setLabelCol(config.getString("labelCol"))
    ...
    if (config.hasPath("featureSubsetStrategy")) {
      gbtClassifier.setFeatureSubsetStrategy(config.getString("featureSubsetStrategy"))
    }

    if (config.hasPath("checkpointInterval")) {
      spark.sparkContext.setCheckpointDir(config.getString("checkpointDir"))
      gbtClassifier.setCheckpointInterval(config.getInt("checkpointInterval"))
      gbtClassifier.setCacheNodeIds(true)
    }

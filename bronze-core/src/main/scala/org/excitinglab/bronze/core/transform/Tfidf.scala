 ...
 * TFIDF
 */
class Tfidf extends BaseTransform {
  ...
  override def process(spark: SparkSession, df: Dataset[Row]): Dataset[Row] = {
    ...
    if (config.hasPath("outputCol")) {
      idf.setOutputCol(config.getString("outputCol"))
    }
    if (config.hasPath("minDocFreq")) {
      idf.setMinDocFreq(config.getInt("minDocFreq"))
    }
    idf.fit(df).transform(df)
  }
